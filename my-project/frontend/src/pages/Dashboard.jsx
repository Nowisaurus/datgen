...
// --- (Constants like DEPARTMENT_TEMPLATES, FONT_FAMILIES, generateDataForRow remain the same) ---
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/users/${userId}`)
      ...
      .then((user) => { if (user?.plan) setUserPlan(user.plan); })
      .catch(() => {});
  }, []);
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition-colors duration-500">
        ...
        <div className="p-4 sm:p-6 md:p-8 lg:p-10 max-w-[95vw] lg:max-w-7xl mx-auto flex flex-col gap-6">
          ...
          <div className="overflow-auto border rounded-lg bg-gray-200 dark:bg-gray-700 p-2 mb-4 flex flex-nowrap gap-2 items-center justify-start">
            ...
            {/* ✅ [FIX] Changed span to a clickable button */}
            <button
              onClick={upsell}
            ...
            {/* ✅ [FIX] Button text is now dynamic */}
        ...
        {showPaypal && (<div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50"><div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg w-[400px]"><h2 className="text-xl font-bold mb-4">Upgrade to Pro</h2><p className="mb-4">Unlock higher row limits and advanced features.</p><Paypalbutton onSuccess={async (data) => {try {const userId = localStorage.getItem("userId"); const res = await fetch("http://localhost:5000/api/paypal/upgrade", {method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({subscriptionID: data.subscriptionID, userId})}); const result = await res.json(); if (result.success) {setUserPlan("pro"); alert("🎉 Your plan has been upgraded!");} else {alert("⚠️ Subscription not active yet.");}} catch (err) {console.error(err); alert("❌ Upgrade failed.");} finally {setShowPaypal(false);}}} onCancel={() => setShowPaypal(false)} /><button onClick={() => setShowPaypal(false)} className="mt-4 px-4 py-2 rounded bg-gray-300 dark:bg-gray-700 hover:bg-gray-400 dark:hover:bg-gray-600 w-full">Cancel</button></div></div>)}
        <UpgradeModal isOpen={showUpgradeModal} onClose={() => setShowUpgradeModal(false)} onUpgrade={() => {setShowUpgradeModal(false); setShowPaypal(true);}}/>
      </div>
    </div>
  );
}